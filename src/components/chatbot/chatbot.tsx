...
const getChats = (chat:Chat,index: number)=>{
    if(chat.sender === 'USER') {
        ...
        <div className="flex gap-3 my-4 text-gray-600 text-sm flex-1" key={index}><span
            ...
            <p className="leading-relaxed"><span className="block font-bold text-gray-700">You </span>{chat.message}</p>
        </div>
        )
    }
    else {
        ...
        <div className="flex gap-3 my-4 text-gray-600 text-sm flex-1"><span
            ...
            <p className="leading-relaxed"><span className="block font-bold text-gray-700">SYC </span>{chat.message}
            </p>
        </div>
        )
    }
...
const chatbot = () => {
   ...
   const handleUserQuery = ()=>{
        ...
        setQuery('');
        run(copyQuery)
        .then(response => setChats((p)=>[...p,{sender: 'AI', message: response}]))
  ...
  return (
    ...
    <div style = {{boxShadow: '0 0 #0000, 0 0 #0000, 0 1px 2px 0 rgb(0 0 0 / 0.05)', display: showChats ? 'block' : 'none', zIndex:100, overflowY:'scroll'}}
        ...
        <Image src={"/cross.png"} alt="logo" width={30} height={30} style={{marginLeft:'360px'}}></Image>
        </button>
        <div className="flex flex-col space-y-1.5 pb-6">
        <h2 className="font-semibold text-lg tracking-tight">SYC AI</h2>
        <p className="text-sm text-[#6b7280] leading-3">Your Mental Health AI Guide</p>
        </div>
        <div className="pr-4 h-[474px]" style={{ minWidth: '100%', display: 'table' }}>
        {
            chats.map((c,i)=>getChats(c,i))
        }
        </div>
        ...
        <form className="flex items-center justify-center w-full space-x-2">
            ...
            placeholder="Type your message" value={query} onChange={(e)=>setQuery(e.target.value)}>
            </input>
            <button
            onClick={(e)=>{
                e.preventDefault();
